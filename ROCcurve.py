# -*- coding: utf-8 -*-
"""
MAIN CROSS-VALIDATION SCRIPT + ROC CURVE
...
if os.path.exists(CLEAN_PATH):
    ...
    df = load_and_clean(PATH)  # ja es guarda automàticament
...
for f in range(N_FOLDS):
    ...
    for _, row in test.iterrows():
        ...
        # Ítems negatius (implicit feedback)
        user_items = set(train[train["userID"] == u]["itemID"])
        negative_items = list(all_items - user_items - {true_item})
        if len(negative_items) > NEGATIVE_SAMPLES:
            negative_items = random.sample(negative_items, NEGATIVE_SAMPLES)
        ...
        if pred != 0:
            ...
            roc_data["knn"]["y_true"].append(1)
            roc_data["knn"]["y_score"].append(pred)

        for ni in negative_items:
            score = predict_knn(u, ni, knn_model)
            if score != 0:
                roc_data["knn"]["y_true"].append(0)
                roc_data["knn"]["y_score"].append(score)
        ...
        roc_data["svd"]["y_true"].append(1)
        roc_data["svd"]["y_score"].append(pred)

        for ni in negative_items:
            score = predict_svd(u, ni, svd_model, svd_users, svd_items, svd_u2i, svd_i2i)
            roc_data["svd"]["y_true"].append(0)
            roc_data["svd"]["y_score"].append(score)
        ...
        roc_data["svd_bias"]["y_true"].append(1)
        roc_data["svd_bias"]["y_score"].append(pred)

        for ni in negative_items:
            score = predict_svd(u, ni, svd_bias_model, sb_users, sb_items, sb_u2i, sb_i2i)
            roc_data["svd_bias"]["y_true"].append(0)
            roc_data["svd_bias"]["y_score"].append(score)
...
plt.title("ROC Curve — Cross-Validation (Implicit Feedback)")
plt.legend()
plt.grid()
plt.show()
